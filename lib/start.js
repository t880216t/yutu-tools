...
async function createBrowser(options, callback) {
    ...
    if (capabilities.browserName === 'chrome'){
        if(options.isMainBrowser){
            ...
            if (["qq", "sougou"].indexOf(options.displayName) > -1){
                const freePort = await detectPort(9222)
                capabilities['goog:chromeOptions']["args"].push(`--remote-debugging-port=${freePort}`)
            }
        }
        if (options.binary){
