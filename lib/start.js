    ...
    inquirer.prompt(questions).then(function(answers){
        ...
        var cmdQueue = async.priorityQueue(function(cmdInfo, next) {
            ...
            arrTasks.push(async function(callback){
                function doNext(browserName='', version='', sessionId='' ){
                    saveTestCode(true, '',browserName, version, sessionId, syncActionId, `${syncDiffBasePath}/diff_${syncActionId}_${sessionId}.png`);
                }
                function catchError(error, browserName='', version='', sessionId=''){
                    saveTestCode(false, error, browserName, version, sessionId, syncActionId, `${syncDiffBasePath}/diff_${syncActionId}_${sessionId}.png`);
                }
                pushRawCmd(cmd, data);
                var arrCodes = [];
                if(mobile){
                        ...
                        case 'expect':
                            ...
                            syncCheckBrowserDrivers && syncCheckBrowserDrivers.forEach(browser => co(function*(){
                                ...
                                if(expectType === 'count'){
                                    ...
                                    const checkImgName = `${syncActionId}_${browser.lastBrowser?.sessionId}.png`
                                    ...
                                    if (checkImg && syncMainImg){
                                        ...
                                        const sendData = {
                                            sessionId: browser.lastBrowser?.sessionId,
                                            syncActionId: syncActionId,
...
async function saveImage(imgBuff, imgPath) {
    return new Promise(function(resolve, reject){ //做一些异步操作
        imgBuff.getDiffImage()
            .pack()
