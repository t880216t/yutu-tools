...
async function createBrowser(options, callback) {
    ...
    if (capabilities.browserName === 'chrome'){
        ...
        if (options?.userDataDir){
            capabilities['goog:chromeOptions']["args"].push(`--user-data-dir=${options.userDataDir}`)
            if (options?.userProfile){
                capabilities['goog:chromeOptions']["args"].push(`--profile-directory=${options.userProfile}`)
            }
        }
    }
    ...
    function onReady(serverAddress, serverPort){
        ...
        setTimeout(() => {
            createBrowser({
                ...
                userDataDir: configJson.webdriver.mainBrowser.userDataDir,
                userProfile: configJson.webdriver.mainBrowser.userProfile,
                isMainBrowser: true,
            ...
            configJson.webdriver?.syncBrowsers.map(browserInfo => {
                setTimeout(function(){
                    createBrowser({
                        ...
                        userProfile: browserInfo.userProfile,
                        isMainBrowser: false,
                        serverAddress: serverAddress,
                        serverPort: serverPort,
