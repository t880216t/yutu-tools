...
function startRecorder(options){
    ...
    var raw = options.raw;
    var defaultOption = options.default || {defaultOpenChecker, defaultBrowserSize};
    ...
    // questions = (configJson.serverIp && configJson.serverIp !== '127.0.0.1') ? [] : questions
    inquirer.prompt(questions).then(function(answers){
        ...
        var continueRecord = answers.continueRecord || false;
        ...
        function saveTestCode(success, error, browserName='', version='', sessionId=''){
        ...
        var cmdQueue = async.priorityQueue(function(cmdInfo, next) {
            ...
            arrTasks.push(function(callback){
                ...
                function doNext(browserName='', version='', sessionId='' ){
                    saveTestCode(true, '',browserName, version, sessionId);
                }
                function catchError(error, browserName='', version='', sessionId=''){
                    saveTestCode(false, error, browserName, version, sessionId);
                }
                if(window !== lastWindowId){
                    lastWindowId = window;
                    lastFrameId = null;
                    pushRawCmd('switchWindow', window);
                    pushTestCode('switchWindow', '', window, 'await driver.sleep(500).switchWindow('+window+');') // browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId, browser.lastBrowser?.sessionId
                    syncCheckBrowserDrivers && syncCheckBrowserDrivers.forEach(browser => browser.sleep(500).switchWindow(window).then(doNext(browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId)).catch(e => catchError(e, browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId)) || doNext(browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId));
                    callback();
                }
                else{
                    callback();
            ...
            arrTasks.push(function(callback){
                ...
                if(frame !== lastFrameId){
                    ...
                    syncCheckBrowserDrivers && syncCheckBrowserDrivers.forEach(browser => browser.switchFrame(null, function(error){
                        if(frame !== null){
                            return this.wait(frame, 10000).then(function(element){
                                return this.switchFrame(element).wait('body');
                            });
                        }
                    }).then(doNext(browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId)).catch(e => catchError(e, browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId)) || doNext(browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId));
                    callback();
                }
                else{
                    callback();
                ...
                if(mobile){
                    ...
                    switch(cmd){
                        case 'url':
                            ...
                            syncCheckBrowserDrivers && syncCheckBrowserDrivers.forEach(browser => browser.url(getVarStr(eval('\`'+data+'\`'))).then(doNext(browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId)).catch(e => catchError(e, browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId)) || doNext(browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId));
                            break;
                        case 'closeWindow':
                            pushTestCode('closeWindow', '', '', 'await driver.closeWindow();');
                            syncCheckBrowserDrivers && syncCheckBrowserDrivers.forEach(browser => browser.closeWindow().then(doNext(browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId)).catch(e => catchError(e, browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId)) || doNext(browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId));
                            break;
                        case 'sleep':
                            pushTestCode('sleep', '', data, 'await driver.sleep('+data+');');
                            syncCheckBrowserDrivers && syncCheckBrowserDrivers.forEach(browser => browser.sleep(data).then(doNext(browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId)).catch(e => catchError(e, browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId)) || doNext(browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId));
                            break;
                        case 'waitBody':
                            arrCodes = [];
                            ...
                            syncCheckBrowserDrivers && syncCheckBrowserDrivers.forEach(browser =>
                                browser.sleep(500)
                                    ...
                                    .then(doNext(browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId))
                                    .catch(e => catchError(e, browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId)) || doNext(browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId));
                            break;
                        case 'eval':
                            pushTestCode('eval', '', data.replace(/\n/g, ' \\n '), 'await driver.eval(_(\''+escapeStr(data)+'\'));');
                            syncCheckBrowserDrivers && syncCheckBrowserDrivers.forEach(browser => browser.eval(getVarStr(data)).then(doNext(browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId)).catch(e => catchError(e, browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId)) || doNext(browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId));
                            break;
                        case 'mouseMove':
                            arrCodes = [];
                            ...
                            syncCheckBrowserDrivers && syncCheckBrowserDrivers.forEach(browser => browser.sleep(300).wait(data.path, 10000).sleep(300).mouseMove(data.x, data.y).then(doNext(browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId)).catch(e => catchError(e, browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId)) || doNext(browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId));
                            break;
                        case 'mouseDown':
                            arrCodes = [];
                            arrCodes.push('await driver.sleep(300).wait(\''+escapeStr(data.path)+'\', 30000)');
                            ...
                            syncCheckBrowserDrivers && syncCheckBrowserDrivers.forEach(browser => browser.sleep(300).wait(data.path, 10000).sleep(300).mouseMove(data.x, data.y).mouseDown(data.button).then(doNext(browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId)).catch(e => catchError(e, browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId)) || doNext(browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId));
                            break;
                        case 'mouseUp':
                            arrCodes = [];
                            arrCodes.push('await driver.sleep(300).wait(\''+escapeStr(data.path)+'\', 30000)');
                            ...
                            syncCheckBrowserDrivers && syncCheckBrowserDrivers.forEach(browser => browser.sleep(300).wait(data.path, 10000).sleep(300).mouseMove(data.x, data.y).mouseMove(data.x, data.y).mouseUp(data.button).then(doNext(browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId)).catch(e => catchError(e, browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId)) || doNext(browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId));
                            break;
                        case 'click':
                            arrCodes = [];
                            ...
                            syncCheckBrowserDrivers && syncCheckBrowserDrivers.forEach(browser => browser.sleep(300).wait(data.path, 10000).sleep(300).mouseMove(data.x, data.y).click(data.button).then(doNext(browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId)).catch(e => catchError(e, browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId)) || doNext(browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId));
                            break;
                        case 'touchClick':
                            arrCodes = [];
                            arrCodes.push('await driver.sleep(300).wait(\''+escapeStr(data.path)+'\', 30000)');
                            ...
                            syncCheckBrowserDrivers && syncCheckBrowserDrivers.forEach(browser => browser.sleep(300).wait(data.path, 10000).sleep(300).touchClick().then(doNext(browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId)).catch(e => catchError(e, browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId)) || doNext(browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId));
                            break;
                        case 'dblClick':
                            arrCodes = [];
                            arrCodes.push('await driver.sleep(300).wait(\''+escapeStr(data.path)+'\', 30000)');
                            ...
                            syncCheckBrowserDrivers && syncCheckBrowserDrivers.forEach(browser => browser.sleep(300).wait(data.path, 10000).sleep(300).mouseMove(data.x, data.y).click(0).click(0).then(doNext(browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId)).catch(e => catchError(e, browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId)) || doNext(browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId));
                            break;
                        case 'sendKeys':
                            pushTestCode('sendKeys', '', data.keys, 'await driver.sendKeys(\''+escapeStr(data.keys)+'\');');
                            syncCheckBrowserDrivers && syncCheckBrowserDrivers.forEach(browser => browser.sendKeys(data.keys).then(doNext(browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId)).catch(e => catchError(e, browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId)) || doNext(browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId));
                            break;
                        case 'keyDown':
                            pushTestCode('keyDown', '', data.character, 'await driver.keyDown(\''+escapeStr(data.character)+'\');');
                            syncCheckBrowserDrivers && syncCheckBrowserDrivers.forEach(browser => browser.keyDown(data.character).then(doNext(browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId)).catch(e => catchError(e, browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId)) || doNext(browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId));
                            break;
                        case 'keyUp':
                            pushTestCode('keyUp', '', data.character, 'await driver.keyUp(\''+escapeStr(data.character)+'\');');
                            syncCheckBrowserDrivers && syncCheckBrowserDrivers.forEach(browser => browser.keyUp(data.character).then(doNext(browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId)).catch(e => catchError(e, browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId)) || doNext(browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId));
                            break;
                        case 'scrollTo':
                            pushTestCode('scrollTo', '', data.x + ', ' + data.y, 'await driver.scrollTo('+data.x+', '+data.y+');');
                            syncCheckBrowserDrivers && syncCheckBrowserDrivers.forEach(browser => browser.scrollTo(data.x, data.y).then(doNext(browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId)).catch(e => catchError(e, browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId)) || doNext(browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId));
                            break;
                        case 'scrollElementTo':
                            ...
                            syncCheckBrowserDrivers && syncCheckBrowserDrivers.forEach(browser => browser.sleep(300).wait(data.path, 10000).sleep(300).scrollElementTo(data.x, data.y).then(doNext(browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId)).catch(e => catchError(e, browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId)) || doNext(browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId));
                            break;
                        case 'select':
                            arrCodes = [];
                            arrCodes.push('await driver.sleep(300).wait(\''+escapeStr(data.path)+'\', 30000)');
                            ...
                            syncCheckBrowserDrivers && syncCheckBrowserDrivers.forEach(browser => browser.sleep(300).wait(data.path, 10000).sleep(300).select({
                                type: data.type,
                                value: data.value
                            }).then(doNext(browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId)).catch(e => catchError(e, browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId)) || doNext(browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId));
                            break;
                        case 'acceptAlert':
                            pushTestCode('acceptAlert', '', '', 'await driver.acceptAlert();');
                            syncCheckBrowserDrivers && syncCheckBrowserDrivers.forEach(browser => browser.acceptAlert().then(doNext(browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId)).catch(e => catchError(e, browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId)) || doNext(browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId));
                            break;
                        case 'dismissAlert':
                            pushTestCode('dismissAlert', '', '', 'await driver.dismissAlert();');
                            syncCheckBrowserDrivers && syncCheckBrowserDrivers.forEach(browser => browser.dismissAlert().then(doNext(browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId)).catch(e => catchError(e, browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId)) || doNext(browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId));
                            break;
                        case 'setAlert':
                            pushTestCode('setAlert', '', data.text, 'await driver.setAlert("'+data.text+'");');
                            syncCheckBrowserDrivers && syncCheckBrowserDrivers.forEach(browser => browser.setAlert(data.text).then(doNext(browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId)).catch(e => catchError(e, browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId)) || doNext(browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId));
                            break;
                        case 'uploadFile':
                            arrCodes = [];
                            ...
                            syncCheckBrowserDrivers && syncCheckBrowserDrivers.forEach(browser => browser.sleep(300).wait(data.path, {timeout: 10000, displayed: false})
                                .sleep(300).sendElementKeys(rootPath+'/uploadfiles/'+data.filename).then(doNext(browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId)).catch(e => catchError(e, browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId)) || doNext(browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId));
                            break;
                        // 添加断言
                        case 'expect':
                            ...
                            let syncTimeStr = new Date().getTime().toString()
                            ...
                            syncCheckBrowserDrivers && syncCheckBrowserDrivers.forEach(browser => co(function*(){
                                var element, value;
                                if(expectType === 'count'){
                                    value = yield browser.sleep(sleepTime).wait(expectParams[0], {
                                        timeout: 1000,
                                        noerror: true
                                    }).then(function(elements){
                                        return elements.length;
                                    ...
                                    let syncMainImg = null;
                                    let checkImg = null;
                                    const syncMainImgName = `main_${syncTimeStr}.png`
                                    const checkImgName = `${browser.lastBrowser?.browserName}_${browser.lastBrowser?.version}_${syncTimeStr}_${uuidv4()}.png`

                                    syncMainImg = yield getImage(caseName, recorderBrowser, escapeStr(expectParams[0]), syncMainImgName )
                                    ...
                                    if(reDomRequire.test(expectType)){
                                        element = yield browser.sleep(sleepTime).wait(expectParams[0], 10000);
                                    }
                                    switch(expectType){
                                        case 'val':
                                        ...
                                        case 'css':
                                            ...
                                            break;
                                        case 'url':
                                            value = yield browser.url();
                                            break;
                                        case 'title':
                                            value = yield browser.title();
                                            break;
                                        case 'cookie':
                                            value = yield browser.cookie(expectParams[0]);
                                            break;
                                        case 'localStorage':
                                            value = yield browser.localStorage(expectParams[0]);
                                            break;
                                        case 'sessionStorage':
                                            value = yield browser.sessionStorage(expectParams[0]);
                                            break;
                                        case 'alert':
                                            value = yield browser.getAlert();
                                            break;
                                        case 'jscode':
                                            value = yield browser.eval(expectParams[0]);
                                            break;
                                    }
                                    value.should.not.be.a('error');
                                }
                        ...
                        case 'insertVar':
                           ...
                           if(varinfo.isNew){
                            ...
                            syncCheckBrowserDrivers && syncCheckBrowserDrivers.forEach(browser => browser.sleep(300).wait(data.path, 10000).val(getVarStr(eval('\`'+varinfo.template+'\`'))).then(doNext(browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId)).catch(e => catchError(e, browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId)) || doNext(browser.lastBrowser?.browserName, browser.lastBrowser?.version, browser.lastBrowser?.sessionId));
                            break;
                        // 更新变量
                        case 'updateVar':
                            co(function*(){
                                ...
                                if(syncCheckBrowserDrivers){
                                    for (var browserIndex in syncCheckBrowserDrivers){
                                        ...
                                        var element, value;
                                        if(reDomRequire.test(updateType)){
                                            element = yield browser.sleep(300).wait(updateParams[0], 10000);
                                        }
                                        switch(updateType){
                                            case 'val':
                                                value = yield element.val();
                                            ...
                                            case 'title':
                                                ...
                                                break;
                                            case 'cookie':
                                                value = yield browser.cookie(updateParams[0]);
                                                break;
                                            case 'localStorage':
                                                value = yield browser.localStorage(updateParams[0]);
                                                break;
                                            case 'sessionStorage':
                                                value = yield browser.sessionStorage(updateParams[0]);
                                                break;
                                        }
                                        if(updateRegex){
        ...
        function onReady(serverAddress, serverPort){
            ...
            // recorder browser
            setTimeout(function(){
                createBrowser({wdPort: wdPort, hosts: hosts, wdPacUrl: wdPacUrl, isRecorder: true, debug: debug, browserName: configJson.webdriver.mainBrowser.browserName, version: configJson.webdriver.mainBrowser.version }, function*(browser){
                    if(!mobile){
                        if (browserSize) {
                    ...
                    const extensionUrl = `chrome-extension://${debug ? 'cnjonjkencgodgmehddbjaiobcdmjije' : 'njkfhfdkecbpjlnfmminhmdcakopmcnc'}/${mobile?'mobile':'start'}.html?port=${serverPort}&ip=${serverAddress}`;
                    yield browser.url(extensionUrl);
                    yield browser.eval('document.title="'+__('recorder_browser_title')+' - UIRecorder"');
                    console.log(`${__('recorder_browser_opened')} ${browser.lastBrowser?.sessionId}`.green);
            ...
            if(mobile){
                ...
                configJson.webdriver?.syncBrowsers && configJson.webdriver?.syncBrowsers.map(browserInfo => {
                    setTimeout(function(){
                        createBrowser({ wdPort: wdPort, hosts: hosts, wdPacUrl: wdPacUrl, isRecorder: false, debug: debug, browserName: browserInfo.browserName, version: browserInfo.version }, function*(browser){
                            if(browserSize){
                                yield browser.windowSize(browserSize[0], browserSize[1]);
                            }
                            else {
                            ...
                            console.log(`${__('checker_browser_opened')} ${browser.lastBrowser?.sessionId}`.green);
                            console.log('');
        ...
        function checkAllReady(){
            if(recorderBrowser && ((openChecker && checkerBrowser && syncCheckBrowserDrivers) || (mobile && recorderMobileApp) || !(openChecker || mobile))){
                if(continueRecord){
                    var testFile = path.resolve(fileName);
            ...
            if(syncCheckBrowserDrivers){
                for (var browserIndex in syncCheckBrowserDrivers){
                    ...
                    await new Promise((resolve) => {
                        browser.close(() => {
                            console.log(`${__('checker_browser_closed')} ${browser.lastBrowser?.browserName} ${browser.lastBrowser?.version}`.green);
                            resolve();
                        })
                    })
                }
...
function createBrowser(options, callback) {
    ...
    var driver = new JWebDriver({
        'host': configJson.webdriver.host ||'127.0.0.1',
        'port': configJson.webdriver.port || options.wdPort
    });
    var capabilities = {
...
async function getImage(caseName, driver, domPath, fileName) {
    let casePath = path.dirname(caseName);
    let syncDiffBasePath = rootPath + '/diffbase/' + casePath + '/'+ 'sync'
    mkdirs(syncDiffBasePath)
    let elemShot = await driver.sleep(300).getScreenshot({
        elem: domPath,
        filename: `${syncDiffBasePath}/${fileName}`
    })
    return elemShot
}
