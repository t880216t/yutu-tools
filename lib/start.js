...
async function createBrowser(options, callback) {
    ...
    if (driver){
        driver.browserId = options.browserId
...
async function startRecorder(){
    ...
    var cmdQueue = async.priorityQueue(function(cmdInfo, next) {
        ...
        arrTasks.push(async function(callback){
            function doNext(browser){
                ...
                saveTestCode({success: true, error: '', browserName, browserVersion, sessionId});
            }
            function catchError(error, browser){
                const {browserName, browserVersion, sessionId} = browser
                saveTestCode({success: false, error, browserName, browserVersion, sessionId});
            }
        ...
        arrTasks.push(async function(callback){
            function doNext(browser){
                const {browserName, browserVersion, sessionId} = browser
                saveTestCode({success: true, error: '', browserName, browserVersion, sessionId, syncActionId, syncDiffImgPath: `${syncDiffOutputBasePath}/diff_${syncActionId}_${sessionId}.png`});
            }
            function catchError(error, browser){
                const {browserName, browserVersion, sessionId} = browser
                saveTestCode({success: false, error, browserName, browserVersion, sessionId, syncActionId, syncDiffImgPath: `${syncDiffOutputBasePath}/diff_${syncActionId}_${sessionId}.png`});
    ...
    function onReady(serverAddress, serverPort){
        ...
        setTimeout(() => {
            createBrowser({
                browserId: configJson.webdriver.mainBrowser.browserId,
                ...
                browserSize: configJson.browserSize,
            ...
            configJson.webdriver?.syncBrowsers.map(browserInfo => {
                setTimeout(function(){
                    createBrowser({
                        ...
                        console.log(`consoleParams:{"sessionId": "${browser?.sessionId}", "type": "sync", "browserInfo": "${browserInfo.displayName}:${browserInfo.version}", "browserId": "${browserInfo.browserId}"}`);
                        console.log('');
    ...
    function saveTestCode(options){
        const {success, error, browserName, browserVersion, sessionId, syncActionId, syncDiffImgPath, } = options
        let title = ''
        if(success){
            console.log(`   ${symbols.ok.green+__('exec_succeed').green} ${browserName} ${browserVersion}`);
            title = lastTestTitle;
        }
        else{
            console.log(`   ${symbols.err.red+__('exec_failed').red} ${browserName} ${browserVersion}`, error && error.message || error);
